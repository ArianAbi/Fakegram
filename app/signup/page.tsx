...
export default function Signup() {
  ...
  const onFormSubmit = async (e: any) => {
    ...
    const { data: _signupData, error: _signupErr } = await supabase.auth.signUp(
      { email: email, password: password }
    );
    const userId = _signupData.user?.id;
    const { error: _inserErr } = await supabase
      .from("users")
  ...
  return (
    <>
      <div className="w-full h-full text-white flex flex-col items-center justify-center px-10 absolute">
        <form
          className="shadow-md rounded-md w-full max-w-[468px] mx-auto p-5 flex flex-col gap-4 text-center sm:py-10 sm:px-6 sm:bg-platform border-2 sm:border-platform"
        ...
        >
          ...
          {/* username Error */}
          {usernameErr ? (
            <span className="text-red-500 text-sm italic my-1 ">
              username exists
            </span>
          ) : (
            <></>
          ...
          {/* error */}
          {error ? (
            <span className="text-red-500 text-sm italic my-1 ">{error}</span>
          ) : (
            <></>
          )}
          ...
          {/* login */}
          <span className="w-full text-center flex items-center justify-center gap-1 text-sm mt-3">
            you have an account ?
